...
name: humanizer-zh
description: >-
  编辑中文文本，去除 AI 写作痕迹，让内容更自然、直接、具体。适用于改写、
  审阅或润色 AI 生成文本，也适用于把过度模板化的中文文案改得更像真实作者。
metadata:
  trigger: 去 AI 味, AI 写作去痕, 中文润色, 人性化改写, 文案改写, 审稿
  source: 翻译自 blader/humanizer，参考 hardikpandya/stop-slop，并适配 Codex skill layout
---
...
默认只返回改写后的文本，除非用户明确要求解释、打分、逐条指出问题或保留修改痕迹。

成功标准：

- 原意、事实、论点、数据、专有名词和语气边界保持不变。
- 去掉模板化表达、空泛夸大、假深度、机械节奏和过度总结。
- 文本更具体、更直接，更像真实作者在表达。
- 不新增来源、案例、指标、承诺或未在原文出现的事实。
- 不把正式文本改成随意聊天，除非用户明确要求更口语。
...
- 人名、地名、机构名、产品名、论文名、书名和品牌名
- 数字、日期、版本号、价格、百分比、样本量和时间线
- 代码、命令、文件路径、API 路径、参数名、标识符和引用文本
- 客户要求、合同边界、研究结论、法律或医学等高风险表述

如果原文含糊、缺少来源或事实可能不成立，直接降低语气或保留不确定性，不要补编证据。
